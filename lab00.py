...
def sigmoid(z: np.ndarray) -> np.ndarray:
    ...
    arr[mask] = 1 / (1 + np.exp(-z[mask]))
    arr[~mask] = np.exp(z[~mask]) / (1 + np.exp(z[~mask]))
...
def cross_entropy(p: np.ndarray, q: np.ndarray) -> float:
    ...
    return float(-np.sum(p * np.log(newq)))
...
def focal_loss(p: np.ndarray, q: np.ndarray, gamma: float = 2.0,
    ...
    newq = np.clip(q, 1e-12, None)
    new_alpha = alpha if alpha is not None else 1
    return float(-np.sum(p * new_alpha * (1 - newq) ** gamma * np.log(newq)))
